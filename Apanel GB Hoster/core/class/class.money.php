...
function cena_slota($slot_12) {
	...
	$uu_info = mysql_fetch_array(mysql_query("SELECT * FROM `klijenti` WHERE `klijentid` = '$_SESSION[user_login]'"));

	$slot_cena = mysql_fetch_array(mysql_query("SELECT `cena` FROM `modovi` WHERE `igra` = '$g_id'"));
...
function cena_slota_code($slot, $game_id, $loc) {
	...
	if ($loc == 'premium1'||$loc == 'premium2'||$loc == 'premium3') {
		...
		$slot_cena = mysql_fetch_array(mysql_query("SELECT `cena` FROM `modovi` WHERE `igra` = '$game_id'"));
		$cena_slota = explode('|', $slot_cena['cena']);
	}
