...
if(!game_perm($conn, server_info($conn, $Server_ID, 'game'), 17) && !game_perm($conn, server_info($conn, $Server_ID, 'game'), 18)) {
	sMSG("Nemate dozvolu za ovu stranicu!", 'error');
	redirect_to(siteURL().'/info/'.$Server_ID);
}

$Page = server_info($conn, $Server_ID, 'name')." - Plugins";
		...
		<div class="container">
			<div class="rows">
				<div class="contect">
					...
					<h2 style="margin-left: 20px;"><i class="fa fa-cogs"></i> Plugini
					...
					$Query = "SELECT * FROM plugins WHERE gameid = '$Game_ID'";

					$PageName = "/plugins/$Server_ID";
					...
					$Data  = "SELECT * FROM plugins WHERE gameid = :gameid ORDER BY name ASC LIMIT $StartLimit, $LimitPerPage";

					$r = $conn->prepare($Data);
					$r->execute(array(':gameid' => $Game_ID));

					while($row = $r->fetch(PDO::FETCH_ASSOC)):
					if(is_plugin_installed($conn, $row['id'], $Server_ID)) {
					?>
					<div class="pmtabel">  
						<div class="col-md-9"><span class="server-name"><i class="fa fa-cog"></i> <?php echo $row['name']; ?></span></div>
						<div class="col-md-17">
							<form action="/process/remove_plugin" method="POST">
								<input type="hidden" name="id" value="<?php echo $Server_ID; ?>" />
								<input type="hidden" name="pluginid" value="<?php echo $row['id']; ?>" />
								<button class="btn btn-primary" style="float: right;"><i class="fa fa-remove"></i> Obrisi plugin </button>
							</form>
						</div>
					...
					<div class="pmtabel">  
						<div class="col-md-9"><span class="server-name"><i class="fa fa-cog"></i> <?php echo $row['name']; ?></span></div>
						<div class="col-md-17">
							<form action="/process/install_plugin" method="POST">
								<input type="hidden" name="id" value="<?php echo $Server_ID; ?>" />
								<input type="hidden" name="pluginid" value="<?php echo $row['id']; ?>" />
