...
if(date('d-H') == "01-00") {
	$serverx = mysql_query("SELECT * FROM `serveri` WHERE `id`=109");
	...
	while($row = mysql_fetch_array($serverx)) {
		...
		stop_server($ip['ip'], $box['sshport'], $server['username'], $server['password'], $serverid, "admin", TRUE);
		reset_server($ip['ip'], $box['sshport'], $server['username'], $server['password'], $serverid, "admin", TRUE);
		start_server($ip['ip'], $box['sshport'], $server['username'], $server['password'], $serverid, "admin", TRUE);
...
function reset_server($ip, $port, $username, $password, $serverid, $klijentid, $restart) {
	global $jezik;
	...
	else 
	{
		...
		{
			$stream = ssh2_shell($con, 'vt102', null, 80, 24, SSH2_TERM_UNIT_CHARS);
			fwrite( $stream, "rm -rf /home/$username/cstrike/addons/amxmodx/data/vault/*".PHP_EOL);
			sleep(1);
			fwrite( $stream, "rm -rf /home/$username/cstrike/addons/amxmodx/data/file_vault/*".PHP_EOL);
			sleep(1);
			fwrite( $stream, "rm -rf /home/$username/cstrike/addons/amxmodx/data/csstats.dat".PHP_EOL);
			sleep(1);
			fwrite( $stream, "rm -rf /home/$username/cstrike/addons/amxmodx/data/toptime.ini".PHP_EOL);
			sleep(1);
