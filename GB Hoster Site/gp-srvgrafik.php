...
$cache_path1 = "grafik/$ipadresa:$port.png";
...
if ((file_exists($cache_path)) && ($mtime < 5)) {
	mysql_close(); header("Content-type: image/png"); readfile($cache_path);
} else {
	$sql = "SELECT igraci, igraci_5min, cache FROM `lgsl` WHERE ip = '$ipadresa' AND q_port = '$port' AND `id` = '$serverid'";
	...
	$crtice = imagecolorallocate($image, 255, 255, 255);
	$brojevi = imagecolorallocate($image, 255, 255, 255);
	$linija_igraca = imagecolorallocate($image, 255, 0, 0);
	
	$ram_grafika = imagecolorallocate($image, 43, 0, 0);
	...
	for($i=0;$i<=$hlin;$i++)
	{
		...
		imageline($image, $margins-1, $y, $tot_width - $margins+6, $y, $crtice);	
		$ylabel = $ymax-2 - intval($vinc * $i / $ratio2);
		$v = intval($vinc * $i / $ratio2);
		imagestring($image,1,5,$y-2,$v,$brojevi);	
	}

	imageline($image, 18, 20, 18, 153, $brojevi);
	imageline($image, 317, 20, 317, 150, $brojevi);
	imageline($image, $margins-5, 150, $tot_width - $margins+6, 150, $brojevi);
	...
	for ($i = 1; $i < 13; $i++)
	{
		...
		imageline($image,$offsetX+7-($i*25),$offsetY-1,$offsetX+7-($i*25),$offsetY-3,$brojevi);
		imageline($image,$offsetX+1-($i*25),$offsetY-3,$offsetX+1-($i*25),$offsetY-4,$brojevi);
		imagestring($image, 1, $offsetX-($i*25), $offsetY+3, $grafik_vreme[$i], $brojevi );
	}
		
	for ($i = 1; $i < 12; $i++) 
	{
		imageline($image,$offsetX-5-($i*25),$offsetY-2,$offsetX-5-($i*25),$offsetY-4,$brojevi);
		imageline($image,$offsetX-12-($i*25),$offsetY-3,$offsetX-12-($i*25),$offsetY-4,$brojevi);
	}
	
	imageline($image,$offsetX-8-($i*25),$offsetY-3,$offsetX-8-($i*25),$offsetY-4,$brojevi);
    ...
    for ($i=1; $i<$count; $i++)
	{
		...
		imagesmoothline($image, $x1 + 6, $y1, $x2 + 6, $y2, $linija_igraca);
		imagelinethick($image, $x1 + 6, $y1, $x2 + 6, $y2, $linija_igraca);
	}
	
	header("Content-type: image/png");
	
	imagefill($image, 0, 0, $ram_grafika);
	imagepng($image);
