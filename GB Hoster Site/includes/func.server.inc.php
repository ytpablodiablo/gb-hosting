...
function ssh_dodaj_server($ip, $port, $username, $password, $novi_user, $novi_user_pw, $mod)
{
	...
	else 
	{
		...
		else 
		{
			...
			$cmd_gamecfg = 'cp -Rf /home/GameFiles/CounterStrike/Public/cstrike/game.cfg /home/'.$novi_user.'/cstrike/;cp -Rf /home/GameFiles/CounterStrike/Public/cstrike/game_init.cfg /home/'.$novi_user.'/cstrike/';
...
function reinstall_server($ip, $port, $username, $password, $serverid, $klijentid)
{
	...
	else 
	{
		...
		else 
		{
			...
			$cmd_test = 'mkdir /home/'.$server["username"].'/';
			$cmd_test2 = 'chown -Rf '.$server["username"].':'.$server["username"].' /home/'.$server["username"].'/';
			...
			$cmd_gamecfg = 'cp -Rf /home/GameFiles/CounterStrike/Public/cstrike/game.cfg /home/'.$server["username"].'/cstrike/;cp -Rf /home/GameFiles/CounterStrike/Public/cstrike/game_init.cfg /home/'.$server["username"].'/cstrike/';
			
			fwrite( $stream, "$cmd_test\n");
			fwrite( $stream, "$cmd_test2\n");
			fwrite( $stream, "$cmd_nice\n");
			fwrite( $stream, "$cmd_wget\n");
			sleep(5);
			fwrite( $stream, "$cmd_unzip\n");
