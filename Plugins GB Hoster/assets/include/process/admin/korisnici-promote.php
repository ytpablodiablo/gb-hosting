...
if (isset($id)) {
		...
		if ($korisnikova_perm['privilegija'] == 0 && $licna_perm['privilegija'] == 2 OR $korisnikova_perm['privilegija'] == 1 && $licna_perm['privilegija'] == 2) {


				$promote = $korisnikova_perm['privilegija']+1;

				$sql = "UPDATE users SET privilegija = '$promote' WHERE user_id = $id ";
				$result = mysqli_query($conn,$sql);

			if ($result) {
					$_SESSION['success'] = 'Uspesno ste promotovali korisnika.';
					header("Location:".siteURL()."/admin.php?admin=korisnici");
					die;
			} else {
					$_SESSION['error'] = 'Dogodila se greska prilikom promotovanja korisnika.';
					header("Location:".siteURL()."/admin.php?admin=korisnici");
					die;				
			}	
				...
				if ($korisnikova_perm['privilegija'] == 0 || $korisnikova_perm['privilegija'] == 2 && $licna_perm['privilegija'] == 1 || $licna_perm['privilegija'] == 2) {
