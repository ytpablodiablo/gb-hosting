...
if(isset($_POST["query"]))
{
 ...
 $query = "
  SELECT * FROM plugini WHERE naziv LIKE '%$search%' AND banned='1' OR original_amxx LIKE '%$search%' AND banned='1' ";
}
else
{
 $query = "
  SELECT * FROM plugini WHERE banned='1' ORDER BY plugin
 ";
}
...
if(mysqli_num_rows($result) > 0)
{
 ...
 while($plugins = mysqli_fetch_array($result))
 {
							...
							$_SESSION['preview_plugin1'] = $plugins['plugin'];
  $output .= '
					...
					';	
						...
						if ($user['privilegija'] == '1' OR $user['privilegija'] == '2') {
								$output .= "<a href='".siteURL()."/assets/include/process/admin/plugini-unban.php?id=".$plugins['plugin']."' id='$plugins[plugin]' class='trash_p' info='$plugins[naziv]' style='cursor:pointer;'><i class='fas fa-ban' style='color: #00FF00;'></i></a>";
						} 
