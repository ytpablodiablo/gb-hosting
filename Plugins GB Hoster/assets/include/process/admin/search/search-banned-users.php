...
if(isset($_POST["query"]))
{
 ...
 $query = "
  SELECT * FROM users WHERE username LIKE '%$search%' AND banned='1' AND deleted_acc='0' OR fullname LIKE '%$search%' AND banned='1' AND deleted_acc='0' ";
}
else
{
 $query = "
  SELECT * FROM users WHERE banned='1' AND deleted_acc='0' ORDER BY user_id
 ";
}
  ...
  $output .= '
					...
					';	
						...
						if ($users['privilegija'] == '0' && $user['privilegija'] == '1' OR $users['privilegija'] == '1' && $user['privilegija'] == '2' OR $users['privilegija'] == '0' && $user['privilegija'] == '2') {
								$output .= "<a href='".siteURL()."/assets/include/process/admin/korisnici-unban.php?id=".$users['user_id']."' id='$users[user_id]' class='unban' info='$users[fullname]' style='cursor:pointer;'><i class='fas fa-ban' style='color: #00FF00;'></i></a>";
						} 
...
{
	if (mysqli_num_rows($result) == 0) {
		echo '<center><b>Ne postoji nijedan korisnik koji je banovan.</b></center>';
	} else {
