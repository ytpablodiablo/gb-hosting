...
if(isset($_POST["query"]))
{
 ...
 $query = "
  SELECT * FROM plugini WHERE naziv LIKE '%$search%' AND aktivan='0' OR original_amxx LIKE '%$search%' AND aktivan='0' ";
}
else
{
 $query = "
  SELECT * FROM plugini WHERE aktivan='0' ORDER BY plugin
 ";
}
...
if(mysqli_num_rows($result) > 0)
{
 ...
 while($plugins = mysqli_fetch_array($result))
 {
							...
							$_SESSION['preview_plugin3'] = $plugins['plugin'];
  $output .= '
					<div id="admin-users-info" class="'.$plugins[plugin].'">
						...
						<b><a href="'.siteURL().'/admin.php?admin=preview_plugin&pluginid='.$_SESSION[preview_plugin3].'" style="font-weight: 600;color: #fff;cursor: pointer;text-decoration: none;font-size: 16px;">'.$plugins['naziv'].'</a></b><br>
