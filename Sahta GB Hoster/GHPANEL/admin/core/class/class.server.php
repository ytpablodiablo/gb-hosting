...
function srv_install($Box_ID, $Srv_Username, $Srv_Password, $Mod_ID) {
	...
	if(!($ssh_conn = ssh2_connect(box_ip($Box_ID), box_ssh($Box_ID)))) {
		...
		if(!ssh2_auth_password($ssh_conn, box_username($Box_ID), box_password($Box_ID))) {
			...
			fwrite($stream, "useradd $Srv_Username\n");
			sleep(1);
			
			// Create user folder
			
			fwrite($stream, "mkdir /home/$Srv_Username\n");
			sleep(1);
			
			// Go to user folder
			
			fwrite($stream, "cd /home/$Srv_Username\n");
			sleep(1);
			...
			fwrite($stream, "screen -mSL ".$Srv_Username."_install_srv\n");
			sleep(1);
			
			// CMD Final - Wget mod files and chown user
			
			$cmd_final = 'curl -o script.sh '.get_mod_link_masina($Mod_ID).' && bash script.sh && chown -Rf '.$Srv_Username.':'.$Srv_Username.' /home/'.$Srv_Username.' && rm -rf screenlog.0 && chmod -R 755 * && exit'; 
...
function srv_delete($Box_ID, $Srv_Username) {
	...
	if(!($ssh_conn = ssh2_connect(box_ip($Box_ID), box_ssh($Box_ID)))) {
		...
		if(!ssh2_auth_password($ssh_conn, box_username($Box_ID), box_password($Box_ID))) {
	    	...
	    	//Add user
			fwrite($stream, "userdel $Srv_Username\n");
			sleep(1);
			//Create user folder
			fwrite($stream, "nice -n 19 rm -Rf /home/".$Srv_Username."".PHP_EOL);
			sleep(2);
	...
	if(!($ssh_conn = ssh2_connect($BOX_IP, $BOX_SSH))) {
		...
		if(!ssh2_auth_password($ssh_conn, server_username($Server_ID), server_password($Server_ID))) {
			...
			if (gp_game_id($Server_ID) == 1) {
				#CS 1.6
				//Stopiraj server
				fwrite($stream, 'kill -9 `screen -list | grep "'.server_username($Server_ID).'" | awk {\'print $1\'} | cut -d . -f1`'.PHP_EOL);
				sleep(2);
				fwrite($stream, 'screen -wipe'.PHP_EOL);
				sleep(1);

				//Startuj server
				fwrite($stream, "screen -A -m -L -S ".server_username($Server_ID)."".PHP_EOL);
				//fwrite($stream, "screen -dmSL '".server_username($Server_ID)."' sh -c \"$S_Command\"".PHP_EOL);
				sleep(1);
				fwrite($stream, "$S_Command".PHP_EOL);
				sleep(2);
				...
				//Startuj server
				fwrite($stream, "screen -A -m -S ".server_username($Server_ID)."".PHP_EOL);
				sleep(1);
				fwrite($stream, "$S_Command".PHP_EOL);
				sleep(2);
...
function reinstall_server($BOX_IP, $BOX_SSH, $BOX_User, $Box_Pass, $Server_ID, $Mod_ID) {
	...
	if(!($ssh_conn = ssh2_connect($BOX_IP, $BOX_SSH))) {
		...
		if(!ssh2_auth_password($ssh_conn, $BOX_User, $Box_Pass)) {
	    	...
	    	$cmd1 = 'screen -m -S '.server_username($Server_ID).'_reinstall';	    
	    	fwrite($stream, "$cmd1\n");
	    	sleep(1);
	    	$cmd2 = 'nice -n 19 rm -Rf /home/'.server_username($Server_ID).'/* && cd /home/'.server_username($Server_ID).'/ && curl -o script.sh '.get_mod_link_masina($Mod_ID).' && bash script.sh && chown -Rf '.server_username($Server_ID).':'.server_username($Server_ID).' /home/'.server_username($Server_ID).' && rm -rf screenlog.0 && chmod -R 755 * && exit';
	    	fwrite($stream, "$cmd2\n");
	    	sleep(45);
