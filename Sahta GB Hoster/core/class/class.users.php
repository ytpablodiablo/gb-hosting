...
function is_verify_key_valid($conn, $Verify_Key) {
	
	$checkUsername = $conn->prepare("SELECT COUNT(*) FROM `users` WHERE `verify` = :verify");
	
	$checkUsername->execute(array(':verify' => $Verify_Key));
...
function is_forgot_key_valid($conn, $Forgot_Key) {
	...
	$checkUsername = $conn->prepare("SELECT COUNT(*) FROM `users` WHERE `forgot` = :forgot");
...
function user_info_with_verify_key($conn, $type, $Verify_Key) {
	
	$GetSQLInfo = $conn -> prepare("SELECT * FROM `users` WHERE `verify` = :verify");
	
	$GetSQLInfo -> execute(array(':verify' => $Verify_Key));
